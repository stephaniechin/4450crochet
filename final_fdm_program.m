function [DOS_initlen,DOS_finlen,DOS_angle] = final_fdm_program(M,N,type,export) 
close all
%clear variables
...
L = 10; %constant
%M = 5; %design variable
%N = 16; %design variable %% need to update calculation of pt_index expression if change N!
%depths = [.5,2,1.5,3,3]
%angles = A/N * ones(1,N); 
%%{
if strcmp(type,'uniform')
    filepre = strcat('initial\uniform',num2str(M),num2str(N))
    depths = L/M * ones(1,M)
    angles = A/N * ones(1,N) 
...
%%
DOS_initlen = DOS(std(initlen),mean(initlen),M);
DOS_finlen = DOS(std(finlen),mean(finlen),M);
DOS_angle = DOS(std(fin_ptangle1),mean(fin_ptangle1),M);
[sort_initlen, I_initlen] = sort(initlen);
[sort_finlen, I_finlen] = sort(finlen);
[sort_angle, I_angle] = sort(fin_ptangle1);
%%
figure(3)
plot(sort_initlen,'or'); hold on;
plot(finlen(I_initlen),'ob'); hold on;
ylabel('edge length')
legend('initial lengths', 'final lengths' )
title('initial and final lengths, sorted by initial length')
if export
    fileout = strcat(filepre,'initlen_distr.jpg');
    saveas(gcf,fileout);
end
%%
figure(4)
plot(sort_finlen,'ob'); hold on;
plot(initlen(I_finlen),'or'); hold on;
ylabel('edge length')
legend('final lengths', 'initial lengths')
title('initial and final lengths, sorted by final length')
if export
    fileout = strcat(filepre,'finallen_distr.jpg');
    saveas(gcf,fileout);
end
%%
figure(5)
plot(sort_angle,'o')
title('angles')
ylabel('projected angle')
%xlabel('node')
if export
    fileout = strcat(filepre,'angle_distr.jpg');
    saveas(gcf,fileout);
end
%%
%%{
